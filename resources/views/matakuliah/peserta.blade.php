...
<head>
    ...
    <title>Peserta {{ $matakuliah->nama_mk }}</title>
...
<body>
    <div class="container">
        <!-- Navigasi -->
        <div class="mb-3">
            <a href="{{ route('matakuliah.index') }}" class="btn btn-outline-secondary btn-back">
        ...
        <div class="header-card">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <h1 class="h3 mb-2">
                        <i class="bi bi-book me-2"></i>{{ $matakuliah->nama_mk }}
                    </h1>
                    <p class="mb-0 opacity-75">
                        <code class="bg-dark bg-opacity-25 p-1 rounded">{{ $matakuliah->kode_mk }}</code> | 
                        <span class="badge bg-light text-dark">{{ $matakuliah->sks }} SKS</span> | 
                        <span class="badge bg-light text-dark">Semester {{ $matakuliah->semester }}</span>
                    </p>
                </div>
                <div class="mt-2 mt-md-0">
                    <span class="badge bg-light text-dark p-3">
                        <i class="bi bi-people-fill me-1"></i>
                        {{ $matakuliah->transkrips->count() }} Peserta
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="table-container">
            ...
            @if($matakuliah->transkrips->count() > 0)
                <div class="table-responsive">
                    <table class="table table-hover table-striped align-middle">
                        ...
                        <tbody>
                            @foreach($matakuliah->transkrips as $index => $transkrip)
                            <tr>
                                <td class="text-center">{{ $index + 1 }}</td>
                                <td>
                                    <code><strong>{{ $transkrip->mahasiswa->nim ?? 'N/A' }}</strong></code>
                                </td>
                                <td>
                                    <a href="{{ route('mahasiswa.show', $transkrip->nim) }}" 
                                       class="participant-link">
                                        <i class="bi bi-person-badge me-1"></i>
                                        {{ $transkrip->mahasiswa->nama ?? 'N/A' }}
                                    </a>
                                </td>
                                <td>
                                    @if($transkrip->mahasiswa->kelas)
                                        <span class="badge bg-info">
                                            <i class="bi bi-building me-1"></i>{{ $transkrip->mahasiswa->kelas }}
                                        </span>
                                    @else
                                ...
                                <td>
                                    <span class="badge bg-secondary">
                                        <i class="bi bi-calendar-week me-1"></i>Semester {{ $transkrip->semester_ambil }}
                                    </span>
                                </td>
                                <td>
                                    @if($transkrip->nilai)
                                        <span class="badge bg-success">
                                            <i class="bi bi-check-circle me-1"></i>{{ $transkrip->nilai }}
                                        </span>
                                    @else
                                ...
                                <td class="text-center">
                                    ...
                                    @endphp
                                    @if($grade != '-')
                                        <span class="badge {{ $gradeClass }} badge-grade">
                                            {{ $grade }}
                                        </span>
                                    @else
                                        <span class="badge bg-secondary">-</span>
                            ...
                            <tr>
                                ...
                                <th colspan="2">
                                    <span class="badge bg-primary p-2">
                                        ...
                                        {{ $matakuliah->transkrips->count() }} Mahasiswa
                                    </span>
                                </th>
                            </tr>
                        </tfoot>
                ...
                <div class="info-box">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            @endphp
                            <p class="mb-1">
                                <strong>Sudah dinilai:</strong> {{ $withNilai }} dari {{ $total }} mahasiswa
                            </p>
                            @if($withNilai > 0)
                                <p class="mb-0">
                                    <strong>Rata-rata nilai:</strong> 
                                    <span class="badge bg-primary">{{ number_format($avgNilai, 2) }}</span>
                                </p>
                            @endif
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="d-flex gap-2 flex-wrap">
                                @foreach(['A', 'B', 'C', 'D', 'E'] as $g)
                                    @if(isset($grades[$g]))
                                        <span class="badge grade-{{ $g }}">{{ $g }}: {{ $grades[$g] }}</span>
                                    @endif
                                @endforeach
                                @if($grades->isEmpty())
